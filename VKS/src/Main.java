...
class MyWindowListener extends WindowAdapter {
    ...
    public void windowOpened(WindowEvent e) {
        Main.area.setText("Приложение открыто");
    }


    @Override
    public void windowDeiconified(WindowEvent e) {
        Main.area.setText("Окно было свернуто");

    }

    @Override
    public void windowActivated(WindowEvent e) {
        //Main.area.setText("Окно развернуто");

    }

    @Override
    public void windowDeactivated(WindowEvent e) {
        Main.area.setText("Окно перестало быть активным");
...
class MyMouseAdapter2 extends MouseAdapter { //создаем свой класс-слушатель, наследуя его от
    ...
    public void mouseEntered(MouseEvent e) {
        Main.myLabel.setText(e.getComponent().getClass().toString());
    }
}

class Start extends MouseAdapter { //создаем свой класс-слушатель, наследуя его от
    //MouseAdapter, что позволяет нам переопределить только нужный нам метод
    public void mouseEntered(MouseEvent e) {
        BufferedImage image = null;
        try {
...
class Stop extends MouseAdapter { //создаем свой класс-слушатель, наследуя его от
    //MouseAdapter, что позволяет нам переопределить только нужный нам метод
    public void mouseEntered(MouseEvent e) {
        BufferedImage image = null;
        try {
